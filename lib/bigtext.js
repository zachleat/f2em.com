        ...
        init: function($head)
        {
            if(!$('#'+BigText.GLOBAL_STYLE_ID).length) {
                $head.append(BigText.generateStyleTag(BigText.GLOBAL_STYLE_ID, ['.bigtext > * { font-size: ' + BigText.STARTING_PX_FONT_SIZE + 'px; }',
                                                                                '.bigtext .' + BigText.EXEMPT_CLASS + ' { font-size: inherit; }']));
            }
        },
        ...
        return this.each(function()
        {
            ...
            if(options.resize) {
                ...
                if($.isFunction(options.bindResize)) {
                    ...
                    $(window).unbind('resize.bigtext-' + id).bind('resize.bigtext-' + id, $.throttle(100, resizeFunction));
                } else if($.fn.smartresize) {
                    // https://github.com/lrbabe/jquery-smartresize/
                    $(window).unbind('smartresize.bigtext-' + id).bind('smartresize.bigtext-' + id, resizeFunction);
                }
            }
            ...
            $t.find(childSelector).each(function(lineNumber)
            {
                $(this).each(function()
                    ...
                    .addClass(BigText.LINE_CLASS_PREFIX)
